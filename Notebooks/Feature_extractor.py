...
def filter_data(df, class_number):
    data = df.copy()
...
def create_model():
    ...
    l1 = tf.keras.layers.Dense(20, activation='relu')(added)
...
nb_classes = 11
nb_model_per_class = 20
model_pool = {i: [] for i in range(nb_classes)}

for i in range(nb_classes):
    df_train = filter_data(df_train, i)
    for j in range(nb_model_per_class):
        data = df_train.sample(frac=0.8)
        x = data.signal
